...
export default async function globalSetup(config: FullConfig) {
  const baseURL =
    ...
    "http://127.0.0.1:3000";
  const email =
    process.env.MUSTER_LOCAL_ADMIN_EMAIL ?? "justin.middler@yuma.example";
  const password =
    process.env.MUSTER_LOCAL_ADMIN_PASSWORD ?? "MusterDemo!2026";
  ...
  if (response.status() === 401) {
    const signup = await api.post("/api/auth/sign-up/email", {
      data: { name: "Justin Middler", ...credentials },
    });
