  ...
  return (
    <main className="grid min-h-dvh bg-background wide:grid-cols-[minmax(0,1.1fr)_minmax(28rem,0.9fr)]">
      <section className="hidden border-r p-12 wide:flex wide:flex-col wide:justify-between">
        <Image src="/muster-logo.png" alt="Muster" width={48} height={48} className="size-12 rounded-lg" priority />
      ...
      <section className="grid place-items-center p-5">
        <div className="w-full max-w-sm">
          <Image src="/muster-logo.png" alt="Muster" width={48} height={48} className="mb-8 size-12 rounded-lg wide:hidden" priority />
          <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Self-hosted security operations</p>
          <h2 className="mt-2 font-display text-2xl font-bold">Sign in to Muster</h2>
          <p className="mt-2 text-sm text-muted-foreground">Sign in with your organisation account.</p>
          <LoginForm />
          <p className="mt-6 text-center text-[11px] leading-5 text-muted-foreground">MFA, recovery codes, passkeys, OIDC, and Entra policies are supported by the authentication architecture.</p>
        </div>
      </section>
    </main>
  );
