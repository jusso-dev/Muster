...
export const EvidenceUploadRequestSchema = z.object({
  ...
  fileName: z.string().min(1).max(255).refine((value) => !/[\/\\\0]/.test(value), "Unsafe file name"),
...
export interface ObjectStorageAdapter {
  presignUpload(storageKey: string, mimeType: string, size: number, expiresSeconds: number): Promise<string>;
  presignDownload(storageKey: string, expiresSeconds: number): Promise<string>;
  quarantine(storageKey: string): Promise<void>;
}

export function evidenceStorageKey(organisationId: string, evidenceId: string, fileName: string) {
